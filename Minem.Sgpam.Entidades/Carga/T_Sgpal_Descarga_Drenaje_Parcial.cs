...
namespace Minem.Sgpam.Entidades
{
    ...
    /// </summary>
    public partial class T_Sgpal_Descarga_Drenaje 
    { 
        public T_Sgpal_Descarga_Drenaje()
        {

        }

        public T_Sgpal_Descarga_Drenaje(IDataReader vRdr)
        {
            			if (!Convert.IsDBNull(vRdr["USU_MODIFICA"]))  			{				this.USU_MODIFICA = Convert.ToString(vRdr["USU_MODIFICA"]);  			}			this.FLG_ESTADO = Convert.ToString(vRdr["FLG_ESTADO"]);  
			if (!Convert.IsDBNull(vRdr["FEC_MODIFICA"]))  			{				this.FEC_MODIFICA = Convert.ToDateTime(vRdr["FEC_MODIFICA"]);  			}			this.ID_DESCARGA_DRENAJE = Convert.ToInt32(vRdr["ID_DESCARGA_DRENAJE"]);  
