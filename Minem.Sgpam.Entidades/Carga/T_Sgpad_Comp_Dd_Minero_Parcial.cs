...
namespace Minem.Sgpam.Entidades
{
    ...
    { 
        ...
        public T_Sgpad_Comp_Dd_Minero(IDataReader vRdr)
        {
			...
			if (!Convert.IsDBNull(vRdr["DES_TIPO"]))
			{				this.DES_TIPO = Convert.ToString(vRdr["DES_TIPO"]);
			}

			if (!Convert.IsDBNull(vRdr["DES_SUSTANCIA"]))
			{				this.DES_SUSTANCIA = Convert.ToString(vRdr["DES_SUSTANCIA"]);
			}
			if (!Convert.IsDBNull(vRdr["DES_SITUACION"]))
			{				this.DES_SITUACION = Convert.ToString(vRdr["DES_SITUACION"]);
			}
			if (!Convert.IsDBNull(vRdr["DES_ESTADO"]))
			{				this.DES_ESTADO = Convert.ToString(vRdr["DES_ESTADO"]);
			}
		}
	}
}